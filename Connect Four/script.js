...
const aiMove = () => {
  setTimeout(() => {
    const winningMove = calculateBestMove(2); // Check if AI can win
    const blockingMove = calculateBestMove(1); // Check if AI can block the player
    const col =
      winningMove !== null
        ? winningMove
        : blockingMove !== null
        ? blockingMove
        : generateRandomNumber(0, 7); // Random move if no strategic move is found

    if (findEmptyRow(col) !== -1) {
      setPiece(col, true);
    } else {
      aiMove(); // Retry if the chosen column is full
    }
